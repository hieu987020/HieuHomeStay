 ...
 * @author hieu
 */
public class CustomerViewDetailBillController extends HttpServlet {
...
//            System.out.println("listSer" + listSer.size());
            ...
            if (isApproved) {
                ...
                BillDTO billDTO = billDAO.loadByPrimaryKey(billID);
                request.setAttribute("ROOMDETAIL", listRoom);
                request.setAttribute("PRODUCTDETAIL", listPro);
                request.setAttribute("SERVICEDETAIL", listSer);
                request.setAttribute("BILL", billDTO);
                request.setAttribute("APPROVE", true);
            } else {
                request.setAttribute("APPROVE", false);
            }

        } catch (Exception e) {
            log("Error at CustomerViewDetailBillController" + e.getMessage());
        } finally {
            request.getRequestDispatcher("customer/mybilldetail.jsp").forward(request, response);
        }
    }
